...
Notation "x <o> y" := (t_iff x y) (at level 95, no associativity).
...
(* this build a proof object of T <o> T[b/a] where p is the proof of a<o>b.
*)
    ...
    | _ =>
     ...
     let H2 := type of name in
         match H2 with
           ...
           | ?H -> ?H2 =>
             clear name;
               assert (H <o> H2) as Hn by (apply T)
    ...
    | _ =>
     ...
     let H2 := type of name in
         match H2 with
           ...
           | ?H -> ?H2 =>
             clear name;
               assert (H2 <o> H) as Hn by (apply T)
...
Theorem trw_demo2:
  forall (P Q: nat -> Type),
    (forall n, P n <o> forall m, Q m )
    -> ((P 1 * void -> void ) <o> ((forall m, Q m) * void -> void )).
Proof.
  intros ? ?  Hiff.
  constructor; intros.
  destruct X; auto.
...
(*
Lemma test :
  forall a b (*c f*), (a <o> b) -> Cast a. (*.  ## { x : c | f x }.*)
Proof.
  intros.
  rewrite X.
Qed.
...
Theorem dont_touch_forall :
  forall (P Q : (nat : Set) -> Type),
    (forall (n : (nat : Set)), P n <o> (forall (m : (nat : Set)), Q (m+n)))
    -> P (1 : (nat : Set))
