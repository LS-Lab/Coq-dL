...
(**
  ...
  Differential invariants --- see Figure 3, Section 5.1

  Not proved yet because we don't know how to interpret
  KFdifferentialFormula.  Instead, this axiom has been replaced by a
  collection of axioms, one for each kind of formula.

 *)
Definition differential_invariants_axiom : Formula :=
  KFimply
    (KFimply
       (Pof1 predq tvarx)
       (KFbox
          (KPodeSystem
             ...
             (KFdifferentialFormula (Pof1 predp tvarx))
       )
    )
    (KFequiv
       (KFbox
       ...
       (KFbox
          (KPtest (Pof1 predq tvarx))
       (Pof1 predp tvarx)
       )
    ).

Definition differential_invariants_rule : rule := MkRule [] differential_invariants_axiom.

Lemma differential_invariants_axiom_sound : rule_true differential_invariants_rule.
Proof.
  ...
  split.
  {
    introv IH1. introv. introv IH2.
    destruct IH2.

    pose proof (IH1 w) as h1. clear IH1.
    apply h1. clear h1.

    pose proof (H H1) as h. clear H.
    pose proof (h w) as Z1. clear h.

    (* I think that this can not be proved because of FalseFormulaSem *)
    destruct Z1.
Abort.
